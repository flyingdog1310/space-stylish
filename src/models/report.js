...
export async function getOrder() {
    const [row] = await dbManager.query("SELECT user_id, total FROM orders");
    console.log(row);
    return row;
}

export async function getTotal() {
    const [total] = await dbManager.query("SELECT SUM (total) FROM orders");
    return total;
}

export async function getSoldColor() {
    const [color] = await dbManager.query("SELECT DISTINCT color_code, color_name FROM order_lists;");
    for (let i = 0; i < color.length; i++) {
        const [total] = await dbManager.query("SELECT SUM (qty) FROM order_lists Where color_code=?", [color[i].color_code]);
...
export async function getSoldPrice() {
    ...
    for (let i = 500; i < 2000; i += 20) {
        [price] = await dbManager.query("SELECT SUM (qty) FROM order_lists WHERE price BETWEEN ? AND ?", [i, i + 19]);
...
export async function getTopFive() {
    let allCandidate = [];
    let candidates;
    [candidates] = await dbManager.query("SELECT DISTINCT product_id FROM order_lists;", []);
    for (let i = 0; i < candidates.length; i++) {
        const [total] = await dbManager.query("SELECT SUM (qty) FROM order_lists Where product_id=?", [
    ...
    for (let i = 0; i < topFive.length; i++) {
        ...
        for (let j = 0; j < 3; j++) {
            ...
            const [topFiveTotal] = await dbManager.query("SELECT SUM (qty) FROM order_lists Where product_id=? AND size=?", [
                topFive[i],
                size,
            ]);
